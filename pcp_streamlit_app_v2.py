...
st.set_page_config(page_title="PCP – Dashboard Dinâmico v5", layout="wide")
...
if not logo_path.exists():
    # fallback to mounted path if running locally in this sandbox structure
...
if logo_path.exists():
    colA.image(str(logo_path), use_column_width=True)
colB.title("PCP – Dashboard Dinâmico v5")
...
# -----------------------------
# Tab 5 — Metas Dinâmicas por Subgrupo (celula/familia)
# -----------------------------
with tab5:
    ...
    if not available:
        ...
        # montar tabela de metas com valores únicos
        ...
        if unicos.empty:
            ...
            unicos["Meta (R$)"] = 30000  # default
            metas_edit = st.data_editor(unicos, num_rows="dynamic")
            metas_map = dict(zip(metas_edit[eixo], metas_edit["Meta (R$)"]))

            # agregação por subgrupo e mês
            ...
            # linha de meta média do subgrupo ao longo do tempo
            for key, chunk in g.groupby(eixo):
                ax6.plot(chunk["month"], chunk["meta"], linestyle="--")
            ax6.set_xlabel("Mês")
            ax6.set_ylabel("R$")
            ax6.yaxis.set_major_formatter(FuncFormatter(lambda x, pos: format_brl(x)))
